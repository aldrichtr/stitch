...
Describe "Testing private function Format-ChangelogHeader" -Tags @('unit', 'ChangelogHeader', 'Format' ) {
    Context 'The Format-ChangelogHeader command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Format-ChangelogHeader'
            $tokens, $errors = @()
