...
Describe "Testing private function Format-ChangelogFooter" -Tags @('unit', 'ChangelogFooter', 'Format' ) {
    Context 'The Format-ChangelogFooter command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Format-ChangelogFooter'
            $tokens, $errors = @()
