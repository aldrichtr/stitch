...
Describe "Testing private function Write-MarkdownDocument" -Tags @('unit', 'MarkdownDocument', 'Write' ) {
    Context 'The Write-MarkdownDocument command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Write-MarkdownDocument'
            $tokens, $errors = @()
