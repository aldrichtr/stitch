...
Describe "Testing private function Get-MarkdownDescendant" -Tags @('unit', 'MarkdownDescendant', 'Get' ) {
    Context 'The Get-MarkdownDescendant command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-MarkdownDescendant'
            $tokens, $errors = @()
