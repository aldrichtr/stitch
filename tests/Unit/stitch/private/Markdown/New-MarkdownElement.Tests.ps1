...
Describe "Testing private function New-MarkdownElement" -Tags @('unit', 'MarkdownElement', 'New' ) {
    Context 'The New-MarkdownElement command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'New-MarkdownElement'
            $tokens, $errors = @()
