...
Describe "Testing private function Get-MarkdownFrontMatter" -Tags @('unit', 'MarkdownFrontMatter', 'Get' ) {
    Context 'The Get-MarkdownFrontMatter command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-MarkdownFrontMatter'
            $tokens, $errors = @()
