...
Describe "Testing private function Disable-FeatureFlag" -Tags @('unit', 'FeatureFlag', 'Disable' ) {
    Context 'The Disable-FeatureFlag command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Disable-FeatureFlag'
            $tokens, $errors = @()
