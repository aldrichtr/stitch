...
Describe "Testing public function Test-InInvokeBuild" -Tags @('unit', 'InInvokeBuild', 'Test' ) {
    Context 'The Test-InInvokeBuild command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Test-InInvokeBuild'
            $tokens, $errors = @()
