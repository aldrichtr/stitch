...
Describe "Testing public function Get-BuildTask" -Tags @('unit', 'BuildTask', 'Get' ) {
    Context 'The Get-BuildTask command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-BuildTask'
            $tokens, $errors = @()
