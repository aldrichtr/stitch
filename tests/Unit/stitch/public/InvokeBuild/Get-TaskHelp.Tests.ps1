...
Describe "Testing public function Get-TaskHelp" -Tags @('unit', 'TaskHelp', 'Get' ) {
    Context 'The Get-TaskHelp command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-TaskHelp'
            $tokens, $errors = @()
        ...
        It "It Should have a 'InvocationInfo' parameter" {
                $command.Parameters['InvocationInfo'].Count | Should -Be 1
            }
    }
}
