...
Describe "Testing public function Get-FeatureFlag" -Tags @('unit', 'FeatureFlag', 'Get' ) {
    Context 'The Get-FeatureFlag command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-FeatureFlag'
            $tokens, $errors = @()
