...
Describe "Testing public function Find-BuildConfigurationRootDirectory" -Tags @('unit', 'BuildConfigurationRootDirectory', 'Find' ) {
    Context 'The Find-BuildConfigurationRootDirectory command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Find-BuildConfigurationRootDirectory'
            $tokens, $errors = @()
