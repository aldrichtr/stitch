...
Describe "Testing public function Find-InvokeBuildScript" -Tags @('unit', 'InvokeBuildScript', 'Find' ) {
    Context 'The Find-InvokeBuildScript command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Find-InvokeBuildScript'
            $tokens, $errors = @()
