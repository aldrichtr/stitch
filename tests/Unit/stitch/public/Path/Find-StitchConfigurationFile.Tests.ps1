...
Describe "Testing public function Find-StitchConfigurationFile" -Tags @('unit', 'StitchConfigurationFile', 'Find' ) {
    Context 'The Find-StitchConfigurationFile command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Find-StitchConfigurationFile'
            $tokens, $errors = @()
