...
Describe "Testing public function Find-LocalUserStitchDirectory" -Tags @('unit', 'LocalUserStitchDirectory', 'Find' ) {
    Context 'The Find-LocalUserStitchDirectory command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Find-LocalUserStitchDirectory'
            $tokens, $errors = @()
