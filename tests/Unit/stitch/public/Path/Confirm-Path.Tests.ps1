...
Describe "Testing public function Confirm-Path" -Tags @('unit', 'Path', 'Confirm' ) {
    Context 'The Confirm-Path command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Confirm-Path'
            $tokens, $errors = @()
        ...
        It "It Should have a 'ItemType' parameter" {
                $command.Parameters['ItemType'].Count | Should -Be 1
            }
    }
}
