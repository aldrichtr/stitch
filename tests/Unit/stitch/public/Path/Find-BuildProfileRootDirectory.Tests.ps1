...
Describe "Testing public function Find-BuildProfileRootDirectory" -Tags @('unit', 'BuildProfileRootDirectory', 'Find' ) {
    Context 'The Find-BuildProfileRootDirectory command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Find-BuildProfileRootDirectory'
            $tokens, $errors = @()
