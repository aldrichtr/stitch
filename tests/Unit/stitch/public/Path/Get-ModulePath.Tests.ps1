...
Describe "Testing public function Get-ModulePath" -Tags @('unit', 'ModulePath', 'Get' ) {
    Context 'The Get-ModulePath command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-ModulePath'
            $tokens, $errors = @()
