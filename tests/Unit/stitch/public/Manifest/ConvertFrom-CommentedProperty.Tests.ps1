...
Describe "Testing public function ConvertFrom-CommentedProperty" -Tags @('unit', 'CommentedProperty', 'ConvertFrom' ) {
    Context 'The ConvertFrom-CommentedProperty command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'ConvertFrom-CommentedProperty'
            $tokens, $errors = @()
