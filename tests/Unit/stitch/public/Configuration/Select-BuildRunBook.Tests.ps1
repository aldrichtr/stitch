...
Describe "Testing public function Select-BuildRunBook" -Tags @('unit', 'BuildRunBook', 'Select' ) {
    Context 'The Select-BuildRunBook command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Select-BuildRunBook'
            $tokens, $errors = @()
        ...
        It "It Should have a 'BuildProfile' parameter" {
                $command.Parameters['BuildProfile'].Count | Should -Be 1
            }
    }
}
