...
Describe "Testing public function Convert-ConfigurationFile" -Tags @('unit', 'ConfigurationFile', 'Convert' ) {
    Context 'The Convert-ConfigurationFile command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Convert-ConfigurationFile'
            $tokens, $errors = @()
