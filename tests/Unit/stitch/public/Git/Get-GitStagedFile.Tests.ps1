...
Describe "Testing public function Get-GitStagedFile" -Tags @('unit', 'GitStagedFile', 'Get' ) {
    Context 'The Get-GitStagedFile command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-GitStagedFile'
            $tokens, $errors = @()
