...
Describe "Testing public function Start-GitBranch" -Tags @('unit', 'GitBranch', 'Start' ) {
    Context 'The Start-GitBranch command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Start-GitBranch'
            $tokens, $errors = @()
