...
Describe "Testing public function Get-GitModifiedFile" -Tags @('unit', 'GitModifiedFile', 'Get' ) {
    Context 'The Get-GitModifiedFile command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-GitModifiedFile'
            $tokens, $errors = @()
