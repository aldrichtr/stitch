...
Describe "Testing public function Get-GitUntrackedFile" -Tags @('unit', 'GitUntrackedFile', 'Get' ) {
    Context 'The Get-GitUntrackedFile command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-GitUntrackedFile'
            $tokens, $errors = @()
