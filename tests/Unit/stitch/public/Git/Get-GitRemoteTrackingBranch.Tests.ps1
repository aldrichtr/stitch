...
Describe "Testing public function Get-GitRemoteTrackingBranch" -Tags @('unit', 'GitRemoteTrackingBranch', 'Get' ) {
    Context 'The Get-GitRemoteTrackingBranch command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-GitRemoteTrackingBranch'
            $tokens, $errors = @()
