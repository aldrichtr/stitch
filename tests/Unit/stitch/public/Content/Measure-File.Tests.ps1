...
Describe "Testing public function Measure-File" -Tags @('unit', 'File', 'Measure' ) {
    Context 'The Measure-File command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Measure-File'
            $tokens, $errors = @()
        ...
        It "It Should have a 'Fix' parameter" {
                $command.Parameters['Fix'].Count | Should -Be 1
            }
    }
}
