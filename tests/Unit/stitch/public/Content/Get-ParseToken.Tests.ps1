...
Describe "Testing public function Get-ParseToken" -Tags @('unit', 'ParseToken', 'Get' ) {
    Context 'The Get-ParseToken command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-ParseToken'
            $tokens, $errors = @()
