...
Describe "Testing public function Find-ParseToken" -Tags @('unit', 'ParseToken', 'Find' ) {
    Context 'The Find-ParseToken command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Find-ParseToken'
            $tokens, $errors = @()
        ...
        It "It Should have a 'Pattern' parameter" {
                $command.Parameters['Pattern'].Count | Should -Be 1
            }
