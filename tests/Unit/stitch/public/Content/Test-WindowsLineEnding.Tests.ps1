...
Describe "Testing public function Test-WindowsLineEnding" -Tags @('unit', 'WindowsLineEnding', 'Test' ) {
    Context 'The Test-WindowsLineEnding command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Test-WindowsLineEnding'
            $tokens, $errors = @()
