...
Describe "Testing public function Test-ProjectPath" -Tags @('unit', 'ProjectPath', 'Test' ) {
    Context 'The Test-ProjectPath command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Test-ProjectPath'
            $tokens, $errors = @()
