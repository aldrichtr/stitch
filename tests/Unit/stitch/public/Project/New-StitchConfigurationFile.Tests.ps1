...
Describe "Testing public function New-StitchConfigurationFile" -Tags @('unit', 'StitchConfigurationFile', 'New' ) {
    Context 'The New-StitchConfigurationFile command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'New-StitchConfigurationFile'
            $tokens, $errors = @()
