...
Describe "Testing public function Get-TestItem" -Tags @('unit', 'TestItem', 'Get' ) {
    Context 'The Get-TestItem command is available and is valid' {
        BeforeAll {
            $command = Get-Command 'Get-TestItem'
            $tokens, $errors = @()
